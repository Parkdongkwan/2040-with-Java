...
public class CellMovement {
	...
	/**
	 * This method returns false if there was any modification in cells
	 * if there was any modification( count > 0) , then it will return true
	 * @return
	 */
	...
	private void moveHorizontally(int i, int j, int des, int sign) {
		if (isValidDesH(i, j, des, sign) && !cells[i][des].getModify()) {  //(fIXED) Fixed the auto double merging.
			...
			score += cells[i][des + sign].getNumber();         //(Fixed) Fixed the scoring system by updating the score variable instantly after merging.
